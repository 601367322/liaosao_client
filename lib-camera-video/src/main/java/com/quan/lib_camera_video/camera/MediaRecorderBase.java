package com.quan.lib_camera_video.camera;
...
public abstract class MediaRecorderBase implements Callback, PreviewCallback, IMediaRecorder {
     ...
     * 视频码率
     */
    protected int mVideoBitrate = VIDEO_BITRATE_NORMAL;
    /**
     * 状态标记
     */
    ...
    public MediaRecorderBase(Activity activity) {
    ...
    public void startPreview() {
        ...
        try {
            ...
            camera.setDisplayOrientation(determineDisplayOrientation(activity, mCameraId));
            try {
                ...
                Log.e("Shen", "setPreviewDisplay fail " + e.getMessage());
            }
            ...
            Log.e("Shen", "startPreview fail :" + e.getMessage());
        }
    }
    ...
    protected void setPreviewCallback() {
        ...
        if (size != null) {
            ...
            try {
                ...
                Log.e("Shen", "startPreview...setPreviewCallback...", e);
            }
            Log.e("Shen", "startPreview...setPreviewCallbackWithBuffer...width:" + size.width + " height:" + size.height);
        } else {
    ...
    protected void concatVideoParts() {
        new AsyncTask<Void, Void, Boolean>() {
            ...
            protected Boolean doInBackground(Void... params) {
                //合并ts流
                String cmd = String.format("ffmpeg %s -i \"%s\" -vcodec copy -acodec copy -absf aac_adtstoasc -b:v 380K -f mp4 -movflags faststart \"%s\"", FFMpegUtils.getLogCommand(), mMediaObject.getConcatYUV(), mMediaObject.getOutputTempVideoPath());
                boolean success = UtilityAdapter.FFmpegRun("", cmd) == 0;
                String cmd1 = String.format("ffmpeg -i \"%s\" -b:v 380K -profile:v baseline -preset ultrafast \"%s\"", mMediaObject.getOutputTempVideoPath(), mMediaObject.getOutputTempVideoPath1());
                UtilityAdapter.FFmpegRun("", cmd1);
                if (success) {
                    FFMpegUtils.captureThumbnails(mMediaObject.getOutputTempVideoPath(), mMediaObject.getOutputTempVideoThumbPath(), "300x300");

                    try {
                        new File(mMediaObject.getOutputTempVideoPath()).delete();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                File dir = new File(mMediaObject.getOutputDirectory());
                File[] files = dir.listFiles();
                for (int i = 0; i < files.length; i++) {
                    if (files[i].getName().lastIndexOf(".ts") > 0) {
                        files[i].delete();
                    }
                    if (files[i].getName().lastIndexOf(".mp4") > 0 || files[i].getName().lastIndexOf(".jpg") > 0) {
                        String toPath = "";
                        if (files[i].getName().lastIndexOf(".mp4") > 0) {
                            toPath = mMediaObject.getOutputVideoPath();
                        } else {
                            toPath = mMediaObject.getOutputVideoThumbPath();
                        }
                        FileUtils.fileCopy(files[i].getPath(), toPath);
                        files[i].delete();
                    }
                }
                return success;
